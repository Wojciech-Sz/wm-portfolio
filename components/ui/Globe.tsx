...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  const _buildMaterial = () => {
    ...
    const globeMaterial =
      globeRef.current.globeMaterial() as unknown as {
        color: Color;
        emissive: Color;
        emissiveIntensity: number;
        shininess: number;
    ...
    globeMaterial.emissiveIntensity =
      globeConfig.emissiveIntensity || 0.1;
  ...
  const startAnimation = () => {
    ...
    globeRef.current
      .arcsData(data)
      .arcStartLat(
        (d) => (d as { startLat: number }).startLat * 1
      )
      .arcStartLng(
        (d) => (d as { startLng: number }).startLng * 1
      )
      .arcEndLat(
        (d) => (d as { endLat: number }).endLat * 1
      )
      .arcEndLng(
        (d) => (d as { endLng: number }).endLng * 1
      )
